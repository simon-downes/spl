...
// in case there's any errors in the code below
...
if( !headers_sent() ) {
    header(sprintf("%s 500 Internal Server Error", $_SERVER['SERVER_PROTOCOL'] ?? 'HTTP/1.0'));
}
...
<div id="header" class="<?=$debug ? 'debug' : ''?>">
    <h1><?=$debug ? $err['name'] : 'Internal Service Error'?></h1>
    <?php if( $debug ) {?><p><?=$err['message']?><?= $err['code'] ? " (Code {$err['code']})" : '' ?></p><?php } ?>
</div>
